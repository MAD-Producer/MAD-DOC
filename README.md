...
- 中文原文保留在现有路径，英文页面位于 `pages/en/`，发布路径统一使用 `/en/` 前缀；
- 翻译须保留 Markdown、代码、图片路径、外部链接及原有段落结构；
- 用户名、账号名、网名和真人姓名保持原文；赛事、活动、组织与团队名称优先采用可核验的官方英文；
- 词表中的既有映射为强制规范。新发现的专有名词应记录英文形式、核验依据与复核状态，再纳入长期词表；
- 英文导航不得以中文正文或删节摘要代替完整英文页面；长篇档案应保留完整来源说明，并在可用时同时本地保存原始文件与配图。
