...
不要根据“能启动软件”的最低要求购机。先记录自己的典型项目：分辨率、帧率、素材编码、最长时长、并发软件、常用效果和预算，再对照官方要求。官方要求会随版本更新，本库只保留入口，不硬编码“永远正确”的配置单。

## 项目长期可打开

- 保留软件与插件的准确版本；
- 收集项目使用的字体、LUT、脚本和插件清单，但不要打包无权分发的安装包；
- 将关键动态链接合成为高质量中间文件；
- 输出带透明通道和不带透明通道的关键镜头；
...
继续阅读：[素材与工程](/tools/footage) · [软件选择](/tools/software) · [插件与 AI](/tools/plugins-ai)
